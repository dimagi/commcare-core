...
public class RunnerCoupler extends ParentRunner<Object> {
    ...
    HashMap<Class, ParentRunner> runners = new HashMap<Class, ParentRunner>();
            ...
            for (Annotation a : clazz.getAnnotations()) {
                if (a instanceof ComposedRunner) {
                    ...
                    for(int i = 0; i < runner.runnables().length ; ++i ){
                        runners.put(runner.runnables()[i], (runner.runners()[i]).getConstructor(Class.class).newInstance(clazz));
                    }
                }
            }
        }catch(NoSuchMethodException | InvocationTargetException | InstantiationException | IllegalAccessException sme) {
            throw new RuntimeException("Invalid Composed runner. All composable runners must have a public constructor that accepts a class");
        }
    }

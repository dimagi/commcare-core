...
public class CasePurgeFilter extends EntityFilter<Case> {
    ...
    private void setIdsToRemoveOld(IStorageUtilityIndexed<Case> caseStorage, Vector<String> owners) {
        //Create a DAG. The Index will be the case GUID. The Nodes will be a string array containing
        //[CASE_STATUS, string(storageid)]
        //CASE_STATUS is enumerated as one of STATUS_LIVE, STATUS_CLOSED, STATUS_DEAD 
        DAG<String, String[], Object> g = new DAG<String,String[], Object>();
        ...
        for(IStorageIterator<Case> i = caseStorage.iterate() ; i.hasMore() ; ) {
            ...
            for(CaseIndex index : c.getIndices()) {
                g.setEdge(c.getCaseId(), index.getTarget(), null);
            }
        }
        ...
        while(!selfDetermined.isEmpty()) {
            ...
            if(node[0] == STATUS_LIVE) {
                ...
                Vector<Edge<String, Object>> descendents = g.getChildren(index);
                if(descendents == null) {
                    continue;
                }
                for(Edge<String, Object> dependentIndex : descendents) {
                    ...
                    for(Edge<String, Object> parent : g.getParents(dependentIndex.i)) {
                        if(g.getNode(parent.i)[0] != STATUS_DEAD) {
                            nodeIsSelfDetermined = false;
                            break;
                        }
                    }
                    if(nodeIsSelfDetermined) {
                        selfDetermined.push(dependentIndex.i);
                    }
                }
            }
        }
